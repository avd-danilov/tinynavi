...
def addLinkways(filename: str, dot_on_merc_min: np):  # разбиваем мысленно все запланированное поле 7200Х9600 на квадраты размером по 120х160 метров (формат дисплея 3:4) и создадим массив списков. Списков будет 6000м/30 =
    ...
    for way in range(0, quantity_way):
        ...
        for dot in range(0, way_dots):
            ...
            if xa == 0 and ya == 0:  # А тут, kzkzkz
                xa = xb
                ya = yb
                x = search_rect(dot_on_merc_min, [xb, yb])[4]
                if x < 3600:
                    linklist[x].append(linkway)
                else:
